...
with open("config.json", "r") as f:
    ...
    print(f'Loading {configfile["botname"]}...')
    bot = commands.Bot(command_prefix=configfile["prefix"], status=discord.Status.idle, activity=discord.Game(name='Booting...'))
    ...
    async def on_ready():
        ...
        if servers == 0:
            print(f'Hi! My name is: {configfile["botname"]}!')
    ...
    async def ping(ctx):
        ...
        embed.set_footer(text="Made by Harry Kearney")
    ...
    async def kick(ctx, member:discord.User = None, reason = None):
        if member == None or member == ctx.message.author:
            embed = discord.Embed(title=configfile["botname"])
    ...
    async def botinfo(ctx):
        embed = discord.Embed(title=configfile["botname"])
        embed.add_field(name='Prefix', value=configfile["prefix"])
